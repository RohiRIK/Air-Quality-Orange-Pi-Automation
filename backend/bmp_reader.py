...
class BME680Reader:
    def __init__(self, device_id: Optional[str] = None, read_interval: Optional[float] = None):
        ...
        try:
            ...
            logger.error(f"Failed to initialize BME680: {e}", extra={"device_id": self.device_id})
            self.bme680 = None
    ...
    def _save_baseline(self):
        try:
            with open(BASELINE_FILE, 'w') as f:
                json.dump({"gas_baseline_ohms": self.gas_baseline, "last_updated": datetime.now().isoformat()}, f)
        except Exception as e:
    ...
    def get_air_quality_status(self, score: float) -> str:
        if score >= 90: return "Air is Crisp"
        if score >= 75: return "Good"
        if score >= 60: return "Moderate"
        if score >= 40: return "Stale Air"
        ...
        while not self.stop_event:
            try:
                ...
                if current_gas > self.gas_baseline:
                    ...
                    self._save_baseline()
                    logger.info(f"New cleaner air baseline found: {self.gas_baseline} Ohms")
